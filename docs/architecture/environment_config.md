...
| `TRENDS_DISCOVERY_RULE_CONFIG_PATH` | empty | discovery evaluation rule JSON 경로. 비어 있거나 파일이 없으면 기본 rule 사용 |
...
`TRENDS_DISCOVERY_RULE_CONFIG_PATH`에는 `config/discovery_rules.example.json` 형식의 JSON 파일을 지정할 수 있다. 이 파일로 generic noise term, keep/weak_keep threshold, publish time window, query origin별 score adjustment/minimum rule, classification별 threshold override를 조정한다.
...
TRENDS_DISCOVERY_RULE_CONFIG_PATH=
```
