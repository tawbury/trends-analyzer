...
- 상위 문서: `docs_index.md`
- 관련 문서: `architecture_specification.md`, `module_design.md`, `docs/spec/source_module_spec.md`, `docs/spec/api_spec.md`, `environment_config.md`
- 최종 수정일: 2026-04-15
...
|-----------|-----------|--------------------------|
| `architecture_specification.md`의 Application / UseCase Layer | `src/application/use_cases/` | `module_design.md`, `docs/spec/source_module_spec.md` |
| `architecture_specification.md`의 Trend Core | `src/core/` | `data_contract_draft.md`, `docs/spec/data_model_spec.md` |
| `architecture_specification.md`의 Adapter Layer | `src/adapters/qts/`, `src/adapters/generic/`, `src/adapters/workflow/` | `module_design.md`, `data_contract_draft.md` |
| `architecture_specification.md`의 Integration / n8n Layer | `src/integration/n8n/` | `module_design.md`, `docs/spec/api_spec.md` |
| `architecture_specification.md`의 Runtime / Dispatch Layer | `src/runtime/dispatch/` | `runtime_scheduling_policy.md`, `observability_ops.md` |
| `api_draft.md`의 Endpoint Groups | `src/api/routes/` | `docs/spec/api_spec.md` |
| `data_contract_draft.md`의 Core Signal Model | `src/contracts/core.py` | `docs/spec/data_model_spec.md` |
| `data_contract_draft.md`의 Consumer Payload Model | `src/contracts/payloads.py` | `module_design.md` |
| `runtime_scheduling_policy.md`의 market-hours guard | `src/shared/market_hours.py`, `src/api/dependencies.py`, `src/batch/runner.py` | `runtime_scheduling_policy.md` |
| `environment_config.md`의 env var groups | `src/shared/config.py` 또는 equivalent | `environment_config.md` |
| `observability_ops.md`의 job/correlation tracking | `src/contracts/runtime.py`, `src/shared/logging.py`, `src/db/repositories/` | `observability_ops.md` |
...
구현 계약의 세부 필드는 `docs/spec/api_spec.md`를 우선한다. `api_draft.md`는 endpoint group과 사용 의도 확인용으로 사용한다.
...
| Config loading | `src/shared/config.py` | `environment_config.md` 기준 env var 로딩 |
...
| 코드 변경 | 함께 갱신할 문서 |
|-----------|------------------|
| 새 contract file 추가 | `data_contract_draft.md`, `docs/spec/source_module_spec.md`, 이 문서 |
| 새 UseCase 추가 | `module_design.md`, `docs/spec/source_module_spec.md`, 이 문서 |
| 새 API endpoint 추가 | `api_draft.md`, `docs/spec/api_spec.md`, 이 문서 |
| 새 runtime mode 추가 | `runtime_scheduling_policy.md`, `environment_config.md`, `docs/spec/source_module_spec.md` |
| 새 env var 추가 | `environment_config.md`, 필요 시 `deployment_topology.md` |
| 새 repository/port 추가 | `docs/spec/persistence_spec.md`, `data_contract_draft.md`, `docs/spec/source_module_spec.md` |
| 새 adapter payload 추가 | `module_design.md`, `data_contract_draft.md`, `docs/spec/api_spec.md` |
| n8n dispatch 정책 변경 | `module_design.md`, `runtime_scheduling_policy.md`, `observability_ops.md` |

규칙:

- 코드를 먼저 바꾸고 문서를 나중에 추측해서 맞추지 않는다.
- 계약 변경은 contract 문서와 source module spec을 먼저 맞춘다.
- API 동작 변경은 `docs/spec/api_spec.md`를 우선 갱신한다.
- 새 환경 변수는 `environment_config.md`에 먼저 등록한다.
- 문서와 코드가 충돌하면 `docs_index.md`의 source-of-truth 규칙을 따른다.
