...
|------|------|
| 작업 브랜치 | `main` |
| 목표 | queue summary comparison recommended action aggregation 구현 |
| 최근 완료 | queue summary comparison aggregated/top recommended actions를 구현하고 `python3 -m pytest` 63개 테스트 통과 |
| 완료 조건 | queue summary comparison artifact가 per-hint recommended_actions를 보존하면서 aggregated/top recommended actions를 제공할 수 있음 |
...
| Discovery Human Review Batch CLI | `src/batch/human_review_export.py`, `src/batch/human_review_import.py` | review artifact를 CSV/JSONL 라벨링 큐로 export하고 라벨링된 CSV/JSONL을 append-only human feedback JSONL로 bulk import. latest feedback 표시/제외, re-review 필터, noisy query priority, human review report disagreement preset, calibration_assist bridge preset, multi-signal OR queue export, priority-scored queue ordering, queue export summary manifest/comparison/interpretation hints/action recommendations/aggregation, dry-run summary, CSV BOM 호환을 지원 |
