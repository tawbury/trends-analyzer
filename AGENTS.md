...
|------|------|
| 작업 브랜치 | `main` |
| 목표 | human review report item-level error sample 및 calibration assist 기반 re-review queue export 구현 |
| 최근 완료 | false_keep/false_drop/weak_mismatch item-level samples와 calibration_assist 기반 assist-preset export를 구현하고 `python3 -m pytest` 55개 테스트 통과 |
| 완료 조건 | human review report가 false_keep/false_drop/weak_mismatch sample을 제공하고 export queue가 calibration_assist hint를 직접 re-review 조건으로 사용할 수 있음 |
